...
- ✅ **Dev-token bypass fails the boot, not just the request**: `plugins/auth.ts` `verifyToken()` only accepts `dev:<firebaseUid>:<phone>` when Firebase is unconfigured AND `NODE_ENV !== "production"`; on top of that, `assertNoDevTokenBypass()` (`core/config.ts`, called from `server.ts` before the app is built) refuses to start a production process with any `FIREBASE_*` key missing, naming the accepting function and the env vars to set. No-op in dev/test.
